...
static void get_state_pointers(struct keyvalue fields[NSTATE_FIELDS+1],
  ...
  fields[0].key = "\"turn\"";
  fields[0].value = &(s->turn);

  fields[1].key = "\"points\"";
  fields[1].value = &(s->points);

  fields[2].key = "\"tri_x\"";
  fields[2].value = &(s->tri_x);

  fields[3].key = "\"rotation\"";
  fields[3].value = &(s->rotation);

  fields[4].key = "\"cannon_t\"";
  fields[4].value = &(s->cannon_t);

  fields[5].key = "\"cannon_offset\"";
  fields[5].value = &(s->cannon_offset);

  fields[6].key = "\"coin_x\"";
  fields[6].value = &(s->coin_x);
